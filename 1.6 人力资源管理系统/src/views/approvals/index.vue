...
<script lang="ts">
	...
	export default defineComponent({
		setup(props, ctx) {
			...
			let approvalsList = reactive<Ts.approvalsList[]>([]);
			// 审批数据
			getApprovals({ page: currentPage.value, pageSize: pageSize.value }).then(
				(res) => {
					console.log(res);
					total.value = res.data.data.total;
					res.data.data.rows.forEach((item: Ts.approvalsList) => {
						approvalsList.push(item);
					});
				},
			);
			// 获取审批数据
